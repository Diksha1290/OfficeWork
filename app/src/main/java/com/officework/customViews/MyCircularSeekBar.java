...
public class MyCircularSeekBar extends View {
    ...
    private static final float DEFAULT_END_ANGLE = 90f; // Geometric (clockwise, relative to 3 o'clock)
    private static final float DEFAULT_POINTER_ANGLE = 0;
    private static final int DEFAULT_MAX = 100;
    private static final int DEFAULT_PROGRESS = 0;
    private static final int DEFAULT_CIRCLE_COLOR =Color.rgb(122, 148, 195);;
    private static final int DEFAULT_CIRCLE_PROGRESS_COLOR = Color.rgb(31, 73, 153);
    private static final int DEFAULT_POINTER_COLOR =Color.rgb(31, 73, 153);
    private static final int DEFAULT_POINTER_HALO_COLOR = Color.rgb(31, 73, 153);
    private static final int DEFAULT_POINTER_HALO_COLOR_ONTOUCH =Color.rgb(31, 73, 153);
    ...
    private static final boolean DEFAULT_DISABLE_POINTER = true;
    ...
    /**
     * Start angle of the MyCircularSeekBar.
    ...
    /**
     * End angle of the MyCircularSeekBar.
     * Note: If mStartAngle and mEndAngle are set to the same angle, 0.1 is subtracted
     * from the mEndAngle to make the circle function properly.
     */
    ...
    private Path sadaPath;

    /**
     * Max value that this MyCircularSeekBar is representing.
     */
    private float mMax;

    /**
     * Progress value that this MyCircularSeekBar is representing.
     */
     ...
     * If false, then the View itself determines the size of the MyCircularSeekBar.
     */
     ...
     * Used when touching the MyCircularSeekBar.
     */
    ...
    private boolean alreaady=false;

    /**
     * Initialize the MyCircularSeekBar with the attributes from the XML style.
    ...
    private void initPaints() {
        ...
        mPointerPaint.setStrokeCap( Paint.Cap.values()[Paint.Cap.ROUND.ordinal()]);
        ...
        sadaPaint.setColor(mPointerColor);
        sadaPaint.setStrokeWidth(mPointerStrokeWidth);
        sadaPaint.setStyle(Paint.Style.STROKE);
        sadaPaint.setStrokeJoin(Paint.Join.ROUND);
        sadaPaint.setStrokeCap(mCircleStyle);
    ...
    private void resetPaths() {
        if (mIsInNegativeHalf) {
            ...
            sadaPath.addArc(mCircleRectF, pointerStart, mPointerAngle);
...
//            }
        ...
        if(!alreaady) {
            alreaady=true;
            sadaPath=mCirclePonterPath;
        }
        canvas.drawPath(sadaPath, sadaPaint);
    }

    /**
     * Get the progress of the MyCircularSeekBar.
     * @return The progress of the MyCircularSeekBar.
     */
    ...
    /**
     * Set the progress of the MyCircularSeekBar.
     * If the progress is the same, then any listener will not receive a onProgressChanged event.
     * @param progress The progress to set the MyCircularSeekBar to.
     */
    ...
    public MyCircularSeekBar(Context context) {
        super(context);
        init(null, 0);
    }

    public MyCircularSeekBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public MyCircularSeekBar(Context context, AttributeSet attrs, int defStyle) {
    ...
    /**
     * Listener for the MyCircularSeekBar. Implements the same methods as the normal OnSeekBarChangeListener.
     */
    public interface OnCircularSeekBarChangeListener {

        public abstract void onProgressChanged(MyCircularSeekBar circularSeekBar, float progress, boolean fromUser);

        public abstract void onStopTrackingTouch(MyCircularSeekBar seekBar);

        public abstract void onStartTrackingTouch(MyCircularSeekBar seekBar);
    }
    ...
    /**
     * Set the max of the MyCircularSeekBar.
     ...
     * @param max The new max for the MyCircularSeekBar.
     */
    ...
    /**
     * Get the current max of the MyCircularSeekBar.
